...
export const createLevel = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (error.code === 'P2002') {
      return res.status(409).json({ error: "Un niveau avec ce nom existe déjà" });
    }
    return res.status(500).json({ error: "Erreur lors de la création du niveau" });
  }
}
...
export const deleteLevel = async (req: Request, res: Response) => {
  ...
  try {
    const courseWithLevel = await prisma.course.count({
      where: { levelId: parseInt(levelId, 10) }
    });

    if (courseWithLevel > 0) {
      return res.status(403).json({ 
        error: `Ce niveau est associé à ${courseWithLevel} cours. Impossible de le supprimer.` 
      });
    }
...
export const updateLevel = async (req: Request<{ levelId: string }>, res: Response) => {
  const { levelId } = req.params;
  const { name } = req.body;

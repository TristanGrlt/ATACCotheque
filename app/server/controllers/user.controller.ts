...
import { PrismaClient, AppPermission } from "@prisma/client";
...
export const signupUser = async (
  ...
  try {
    ...
    if (!roleIds || roleIds.length === 0) {
      return res
        .status(403)
        .json({ error: "L'utilisateur doit poséder au moins un role" });
    }
    ...
    if (tusername.length <= 3) {
      return res.status(400).json({
        error: "Le nom d'utilisateur doit contenir au moins 4 caratères",
      });
    }
  ...
  try {
    ...
    if (roleIds) {
      ...
      if (roleIds.length === 0) {
        return res
          .status(400)
          .json({ error: "Un utilisateur doit avoir au moins un rôle" });
      }
      ...
      if (userId === currentUserId) {
        ...
        if (!hasAdminPermission) {
          return res.status(403).json({
            error:
              "Vous ne pouvez pas vous retirer la permission de gérer les utilisateurs.",
          });
        }
      }
